...
#
# run with:
#     run figure_4.py -t pdf -p figure_4 -m 'shared'
...
if __name__ == '__main__':
    ...
    usetex      = False
    serif       = False
    picklefile  = None          # default "results_shared-benchmark-model_xSSA.pkl"
    model       = 'shared'
    ...
    if picklefile is None:
        picklefile = "../examples/data_out/benchmark/results_shared-benchmark-model_xSSA.pkl"
...
#
    ...
    theoretical_results = {}
    theoretical_results['paras']                  = {}
    theoretical_results['paras']['si']            = setup['theo_si_sti_shared']['wa_A-wb_B-wc_C_para'][0]
    theoretical_results['paras']['sti']           = setup['theo_si_sti_shared']['wa_A-wb_B-wc_C_para'][1]
    theoretical_results['process_options']        = {}
    theoretical_results['process_options']['si']  = setup['theo_si_sti_shared']['wa_A-wb_B-wc_C_option'][0]
    theoretical_results['process_options']['sti'] = setup['theo_si_sti_shared']['wa_A-wb_B-wc_C_option'][1]
    ...
    hspace      = 0.22        # x-space between subplots
    ...
    si_numerical  = np.array([ numerical_results[str(iset)]['paras']['si']['out'] for iset in nsets ])
    sti_numerical = np.array([ numerical_results[str(iset)]['paras']['sti']['out'] for iset in nsets ])
    ...
    for ipara,para in enumerate(paras):
        if ipara < int(np.ceil(len(paras)/2.)):
            lstyle = '-'
        else:
            lstyle = '--'
        mark1 = sub.plot(nsets, si_numerical[:,ipara]-theoretical_results['paras']['si'][ipara], alpha=1.0,
                         linewidth=lwidth,
                         linestyle=lstyle,
                         color=colors_benchmark[ipara%int(np.ceil(len(paras)/2.))],
                         marker='o',
                         markeredgecolor=colors_benchmark[ipara%int(np.ceil(len(paras)/2.))],
                         markerfacecolor='None',
                         markersize=3.0, #msize,
                         markeredgewidth=mwidth,
                         label=para)    # Si wmean
    plt.ylabel(str2tex("$S_{i}^{(appr)} - S_{i}^{(theo)}$",usetex=usetex))
    ...
    for ipara,para in enumerate(paras):
        if ipara < int(np.ceil(len(paras)/2.)):
            ...
            lstyle = '--'
        mark2 = sub.plot(nsets, sti_numerical[:,ipara]-theoretical_results['paras']['sti'][ipara], alpha=1.0,
                         linewidth=lwidth,
                         linestyle=lstyle,
                         color=colors_benchmark[ipara%int(np.ceil(len(paras)/2.))],
                         marker='o',
                         ...
                         markersize=3.0, #msize,
                         markeredgewidth=mwidth,
                         label=para )    # STi wmean
    plt.ylabel(str2tex("$ST_{i}^{(appr)} - ST_{i}^{(theo)}$",usetex=usetex))
    abc2plot(sub,dxabc,dyabc,iplot,bold=True,usetex=usetex,mathrm=True, large=True, parenthesis='none',verticalalignment='top')
    plt.title(str2tex("Total Sobol' Index $ST_i$",usetex=usetex))
    ...
    # text to the right
    sub.text(1.04, 0.5, str2tex("Parameters",usetex=usetex),rotation=90, fontsize='large', horizontalalignment='left', verticalalignment='center', transform=sub.transAxes)
    ...
    si_numerical  = np.array([ numerical_results[str(iset)]['process_options']['si']['out'] for iset in nsets ])
    sti_numerical = np.array([ numerical_results[str(iset)]['process_options']['sti']['out'] for iset in nsets ])
    ...
    for iiprocopt,iprocopt in enumerate(procopt):
        if iiprocopt < int(np.ceil(len(procopt)/2.)):
            lstyle = '-'
        else:
            lstyle = '--'
        mark1 = sub.plot(nsets, si_numerical[:,iiprocopt]-theoretical_results['process_options']['si'][iiprocopt], alpha=1.0,
                         linewidth=lwidth,
                         linestyle=lstyle,
                         color=colors_benchmark[iiprocopt%int(np.ceil(len(procopt)/2.))],
                         marker='o',
                         markeredgecolor=colors_benchmark[iiprocopt%int(np.ceil(len(procopt)/2.))],
                         markerfacecolor='None',
                         markersize=3.0, #msize,
                         markeredgewidth=mwidth,
                         label=iprocopt)    # Si wmean
    plt.ylabel(str2tex("$S_{i}^{(appr)} - S_{i}^{(theo)}$",usetex=usetex))
    abc2plot(sub,dxabc,dyabc,iplot,bold=True,usetex=usetex,mathrm=True, large=True, parenthesis='none',verticalalignment='top')
    #plt.title(str2tex("Main Sobol' Index $S_i$",usetex=usetex))
    ...
    for iiprocopt,iprocopt in enumerate(procopt):
        if iiprocopt < int(np.ceil(len(procopt)/2.)):
            ...
            lstyle = '--'
        mark2 = sub.plot(nsets, sti_numerical[:,iiprocopt]-theoretical_results['process_options']['sti'][iiprocopt], alpha=1.0,
                         linewidth=lwidth,
                         linestyle=lstyle,
                         color=colors_benchmark[iiprocopt%int(np.ceil(len(procopt)/2.))],
                         marker='o',
    ...
    # text to the right
    sub.text(1.04, 0.5, str2tex("Process Options",usetex=usetex),rotation=90, fontsize='large', horizontalalignment='left', verticalalignment='center', transform=sub.transAxes)
    ...
    for iiprocesses,iprocesses in enumerate(processes):
        if iiprocesses < int(np.ceil(len(procopt)/2.)):
            ...
            lstyle = '--'
        mark1 = sub.plot(nsets, si_numerical[:,iiprocesses]-theoretical_results['processes']['si'][iiprocesses], alpha=1.0,
                         linewidth=lwidth,
                         linestyle=lstyle,
    ...
    for iiprocesses,iprocesses in enumerate(processes):
        if iiprocesses < int(np.ceil(len(procopt)/2.)):
            ...
            lstyle = '--'
        mark2 = sub.plot(nsets, sti_numerical[:,iiprocesses]-theoretical_results['processes']['sti'][iiprocesses], alpha=1.0,
                         linewidth=lwidth,
                         linestyle=lstyle,
                         color=colors_benchmark[iiprocesses%int(np.ceil(len(procopt)/2.))],
                         marker='o',
    ...
    # text to the right
    sub.text(1.04, 0.5, str2tex("Processes",usetex=usetex),rotation=90, fontsize='large', horizontalalignment='left', verticalalignment='center', transform=sub.transAxes)
